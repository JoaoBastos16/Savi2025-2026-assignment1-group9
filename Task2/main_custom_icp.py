...
def custom_icp(source_pcd, target_pcd, init_params=None,
               max_iterations=20, tolerance=1e-3, point_to_plane=True):
    """
    ...
    for it in range(max_iterations):
        ...
        source_pcd=source_pcd.voxel_down_sample(0.03)
        target_pcd=target_pcd.voxel_down_sample(0.03)
...
def main():
    ...
    init_params = np.array([0.0, 0.0, 0.0, 0.05, 0.0, 0.9])  # pequena translação no eixo X
    ...
    o3d.visualization.draw_geometries([
        pcd_src.paint_uniform_color([1, 0, 0]),
        pcd_tgt.paint_uniform_color([0, 1, 0]),
    ]  ,window_name="ICP Customizado — Final Result",)
